	...
	implements UvMap
{
	RIGHT {
		...
		{
			double u = MathFuncs.mod2(1 - point.getZ()) / 2.0;
			double v = MathFuncs.mod2(point.getY() + 1) / 2.0;
	...
	LEFT {
		...
		public DoublePair map(Point point)
		{
			double u = MathFuncs.mod2(point.getZ() + 1) / 2.0;
			double v = MathFuncs.mod2(point.getY() + 1) / 2.0;
	...
	UP {
		...
		public DoublePair map(Point point)
		{
			double u = MathFuncs.mod2(1 + point.getX()) / 2.0;
			double v = MathFuncs.mod2(1 - point.getZ()) / 2.0;
	...
	DOWN {
		...
		{
			double u = MathFuncs.mod2(1 + point.getX()) / 2.0;
			double v = MathFuncs.mod2(point.getZ() + 1) / 2.0;
	...
	FRONT {
		...
		public DoublePair map(Point point)
		{
			double u = MathFuncs.mod2(point.getX() + 1) / 2.0;
			double v = MathFuncs.mod2(point.getY() + 1) / 2.0;
